...
- **Config: marrow only.** `[cortex.breaker]` in `~/.config/marrow/config.toml` (`enabled` / `fuse_threshold` / `window_hours` / `trip_message`), read by `breaker.settings()`; any key that file leaves unset comes from `mw config --resolved`. No copy in the bridge — an unresolvable key raises `BreakerConfigError`.
...
- **Announcing a trip written by the OTHER shell**: the cli watchdog only writes breaker.json + a marrow alert row — it has no channel — so the chat notice comes from here. `_announce_breaker_trip()` compares the standing auto trip's `ts` against `breaker_announced_ts` in this shell's ledger and, when they differ, stamps the marker then sends `breaker.trip_message()` via `_notify`. Called at TWO checkpoints: unconditionally at the top of `after_turn` (fires within a turn while she is chatting) and in `_fire` just before the `_breaker_holds()` gate (covers the silent path, <= one idle window). Both are needed — an active conversation re-arms the scheduled round on every inbound message, so `_fire` alone could be starved indefinitely.
  - Exactly once per trip: the marker is stamped BEFORE the send, so a dead bot costs that one notice instead of re-announcing forever; a clear stamps `""` and a re-trip announces again. `_record_fuse` stamps the same marker for tg's OWN trips, which is what stops the checkpoint right after it from double-announcing. A manual pause is never announced (reason != `auto_fuse`) — trip_message describes a fuse breach.
  - Marker init (`_init_breaker_marker`, in `__init__`): a trip already standing at the FIRST ever boot is adopted silently, so the bridge never announces one that predates the marker. Once the key exists it is never re-adopted — that is what lets a trip written while the bridge was down still reach her on the first pass. `{count}` comes from `breaker.fuse_count()` (read-only tally, no event recorded).
...
- Config: `synapse_core/config.default.toml` is the single defaults table; `synapse_core/config.py` deep-merges `~/.config/synapse-<channel>/config.toml` over it and `FIELDS` maps attribute → (section, key, channels). `TgConfig`/`Config` are attribute views with no defaults of their own. Inspect with `python -m synapse_core.config --defaults | --resolved --channel tg|wx`. Old `[session]` keys are migrated to `[marrow]`/`[provider]` with a warning. `[cortex].shell_idle_min` is owned by cortex (`[wake].default_sleep_min`) and resolved live via `synapse_core/upstream.py`.
